...
2. Dispatch the `code-reviewer` agent with that diff, or apply its checklist yourself: blueprint layout, service boundaries, VN/EN naming, API conventions, security roles, event/saga correctness, types, tests, migrations (see `.Codex/agents/code-reviewer.md`).
