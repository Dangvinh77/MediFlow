...
```bash
mvn -pl report-service -am spring-boot:run
```
...
```bash
mvn -pl report-service test        # unit (domain + application, no Spring)
mvn -pl report-service verify      # + integration (Testcontainers, needs Docker)
```
