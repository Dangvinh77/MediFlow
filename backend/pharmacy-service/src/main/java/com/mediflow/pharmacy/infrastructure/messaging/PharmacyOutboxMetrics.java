...
/** Publishes retryable and quarantined health gauges for the pharmacy outbox. */
@Component
@ConditionalOnProperty(name = {
        "mediflow.pharmacy.outbox.enabled",
        "mediflow.pharmacy.outbox.metrics-enabled" }, havingValue = "true", matchIfMissing = true)
public class PharmacyOutboxMetrics {
    ...
    public PharmacyOutboxMetrics(PharmacyEventOutboxJpaRepository repository,
        ...
        Gauge.builder("mediflow.pharmacy.outbox.quarantined", quarantined,
                AtomicLong::doubleValue).description("Quarantined pharmacy outbox rows")
                .register(meterRegistry);
        Gauge.builder("mediflow.pharmacy.outbox.oldest-quarantined-age-seconds",
                oldestQuarantinedAgeSeconds, AtomicLong::doubleValue)
                .description("Age of oldest quarantined pharmacy outbox row")
                .register(meterRegistry);
    }
    ...
    public void refresh() {
        ...
        oldestAgeSeconds.set(repository.findFirstByPublishedAtIsNullAndQuarantinedAtIsNullOrderByCreatedAtAsc()
        ...
        oldestQuarantinedAgeSeconds.set(repository
                ...
                .map(row -> Math.max(0L, Instant.now(clock).getEpochSecond()
                        - row.getQuarantinedAt().getEpochSecond()))
                .orElse(0L));
    }
}
