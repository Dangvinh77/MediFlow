...
```bash
mvn -pl billing-service -am spring-boot:run
```
...
```bash
mvn -pl billing-service test        # unit (domain + application, no Spring)
mvn -pl billing-service verify      # + integration (Testcontainers, needs Docker)
```
