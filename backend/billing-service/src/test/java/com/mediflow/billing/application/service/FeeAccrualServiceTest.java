...
class FeeAccrualServiceTest {
    ...
    @Test
    void onLabResult_noLabTypeKnown_keepsEventRetryable() {
        UUID labId = UUID.randomUUID();
        when(processedEvent.alreadyProcessed(any())).thenReturn(false);
        ...
        LabResultCreatedEvent event = new LabResultCreatedEvent(
                UUID.randomUUID(), Instant.now(), "corr", labId, UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID());
        org.assertj.core.api.Assertions.assertThatThrownBy(() -> service.onLabResultCreated(event))
        ...
        when(labTestTypePort.labType(labId)).thenReturn(Optional.of("CBC"));
        when(priceList.labFee("CBC")).thenReturn(new BigDecimal("90000"));
        service.onLabResultCreated(event);
        verify(feeRepo).save(any(Fee.class));
        verify(processedEvent).markProcessed(event.eventId(), "lab.result.created");
    }

    @Test
    void onLabResult_usesProducerTypeAndDateWithoutProjection() {
        LocalDate performed = LocalDate.of(2026, 9, 1);
        LabResultCreatedEvent event = new LabResultCreatedEvent(UUID.randomUUID(), Instant.now(), "corr",
                UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(), "CBC", performed);
        when(priceList.labFee("CBC")).thenReturn(new BigDecimal("90000"));
        service.onLabResultCreated(event);
        ArgumentCaptor<Fee> fee = ArgumentCaptor.forClass(Fee.class);
        verify(feeRepo).save(fee.capture());
        assertThat(fee.getValue().getIncurredDate()).isEqualTo(performed);
        assertThat(fee.getValue().getAmount()).isEqualByComparingTo("90000");
        org.mockito.Mockito.verifyNoInteractions(labTestTypePort);
    }
    ...
    @Test
    void arrivedAndRecordCreated_shareOneExamFee() {
        UUID record = UUID.randomUUID(), patient = UUID.randomUUID(), dept = UUID.randomUUID();
        when(feeRepo.existsBySource(FeeType.EXAM, record)).thenReturn(false, true);
        when(priceList.examFee(dept)).thenReturn(new BigDecimal("150000"));
        service.onAppointmentStatusChanged(new AppointmentStatusChangedEvent(UUID.randomUUID(), Instant.now(),
                "corr", UUID.randomUUID(), patient, dept, "ARRIVED", record));
        verify(feeRepo).save(any(Fee.class));
        service.onMedicalRecordCreated(new MedicalRecordCreatedEvent(UUID.randomUUID(), Instant.now(),
                "corr", record, patient, dept, LocalDate.now()));
        verify(feeRepo).save(any(Fee.class));
        verify(processedEvent, times(2)).markProcessed(any(), any());
    }
