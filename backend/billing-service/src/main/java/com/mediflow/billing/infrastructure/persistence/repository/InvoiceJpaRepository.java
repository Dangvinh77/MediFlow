...
public interface InvoiceJpaRepository extends JpaRepository<InvoiceJpaEntity, UUID> {
    ...
    @org.springframework.data.jpa.repository.Lock(jakarta.persistence.LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT i FROM InvoiceJpaEntity i WHERE i.prescriptionId = :prescriptionId")
    Optional<InvoiceJpaEntity> findByPrescriptionForUpdate(@Param("prescriptionId") UUID prescriptionId);
