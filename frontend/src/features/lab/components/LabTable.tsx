...
export function LabTable() {
  ...
  async function loadTests(filters: { departmentId?: string; status?: LabTestStatus } = {}) {
    setLoading(true);
    setError(null);
    try {
      const page = await labApi.search(filters);
    ...
    <section className="mt-6">
      <form onSubmit={onFilter} className="flex flex-col gap-3 lg:flex-row">
        ...
        >
          ...
          {LAB_STATUSES.map((value) => <option key={value} value={value}>{value}</option>)}
        </select>
      ...
      {!loading && !error && tests.length === 0 && <p className="mt-4 text-zinc-500">Chưa có yêu cầu xét nghiệm phù hợp.</p>}
